...
def get_users():
    ...
    # JSON feed for Kivy UI / device
    if request.args.get("format") == "json":
        rows = db.execute(
            "SELECT user_id, name, birthdate FROM users ORDER BY user_id ASC;"
        ).fetchall()

        users = []
        for row in rows:
            name = row["name"] or ""
            # 👇 skip admin user in the UI list
        ...
        return users  # Flask will jsonify this list

    # HTML view → same behavior as before
    if "user" not in session:
        return redirect(url_for("login"))
